...
interface StatisticsData {
  monthlyExpenseData: Array<{ name: string; amount: number }>;
  groupExpenseData: Array<{ name: string; amount: number }>;
  participationRateData: Array<{
    name: string;
...
export default function StatisticsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await authApi.get<StatisticsData>(
          "api/v1/profile/mypage/statistics"
        );
  ...
  const hasData =
    ...
    groupExpenseData.length > 0 ||
    participationRateData.length > 0;
  ...
  if (!hasData) {
    return (
      <>
        ...
        <main className="flex-1 overflow-auto p-4 space-y-6 pb-16">
          <div className="flex items-center justify-center h-full">
            <div className="text-lg text-gray-500">통계 데이터가 없습니다</div>
          </div>
        </main>
      </>
    );
