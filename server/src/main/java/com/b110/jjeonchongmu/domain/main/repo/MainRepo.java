...
public interface MainRepo extends JpaRepository<Schedule, Long> {
    ...
    @Query(value = "SELECT COUNT(*) FROM schedule s " +
            "JOIN schedule_attendee sa ON s.schedule_id = sa.schedule_id " +
            "WHERE sa.user_id = :userId AND sa.is_confirmed = false", nativeQuery = true)
    int countUncheckSchedules(@Param("userId") Long userId);

    /**
     * 미확인 일정 목록 조회
     */
    @Query(value = "SELECT s.schedule_id, s.title, s.detail, s.place, s.start_time, " +
            "g.gathering_id, g.name, COUNT(sa.schedule_attendee_id) " +
            "FROM schedule s " +
            "LEFT JOIN gathering g ON s.gathering_id = g.gathering_id " +
            "JOIN schedule_attendee sa ON s.schedule_id = sa.schedule_id " +
            "WHERE sa.user_id = :userId AND sa.is_confirmed = false " +
    ...
    @Query(value = "SELECT s.schedule_id, s.title, s.detail, s.place, s.start_time, " +
            ...
            "LEFT JOIN gathering g ON s.gathering_id = g.gathering_id " +
            "JOIN schedule_attendee sa ON s.schedule_id = sa.schedule_id " +
            "WHERE sa.user_id = :userId AND s.gathering_id IS NULL " +
            "GROUP BY s.schedule_id, g.gathering_id " +
            "ORDER BY s.start_time", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT DAY(s.start_time) as day, COUNT(DISTINCT s.schedule_id) as schedule_count " +
            "FROM schedule s " +
            "JOIN schedule_attendee sa ON s.schedule_id = sa.schedule_id " +
            "WHERE sa.user_id = :userId " +
            "AND YEAR(s.start_time) = :year AND MONTH(s.start_time) = :month " +
            "GROUP BY DAY(s.start_time) " +
            "ORDER BY DAY(s.start_time)", nativeQuery = true)
