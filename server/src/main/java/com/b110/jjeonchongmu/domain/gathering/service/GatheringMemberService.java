...
public class GatheringMemberService {
    ...
    public void rejectGathering(Long gatheringId, Long userId) {
        ...
        gatheringMemberRepo.deleteByGatheringGatheringIdAndGatheringMemberUserId(gatheringId, userId);
    }
    ...
    /**
     * 모임 회원 관리 정보를 조회 (총무 전용)
     * 회원 목록, 납부 현황 등의 상세 정보를 포함
     * 
     * @param gatheringId 모임 ID
    ...
    public MemberManageResponseDTO getMemberManage(Long gatheringId) {
        ...
        List<MemberManageResponseDTO.MemberDTO> memberDTOs = members.stream()
                .map(member -> {
                    User user = userRepo.findById(member.getGatheringMemberUserId())
        ...
        return MemberManageResponseDTO.builder()
                ...
                .paymentCount((int) members.stream().filter(GatheringMember::isGatheringPaymentStatus).count())
                .inviteCount(0)
                .inviteList(List.of())
                .manager(managerDTO)
